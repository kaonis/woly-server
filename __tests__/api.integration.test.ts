...
describe('API Integration Tests', () => {
  ...
  describe('POST /hosts', () => {
    ...
    it('should return 500 for duplicate MAC address', async () => {
      ...
      await request(app).post('/hosts').send(host1).expect(201);
      await request(app).post('/hosts').send(host2).expect(500);
    });
  });
  ...
  describe('GET /hosts/mac-vendor/:mac', () => {
    it('should return vendor information', async () => {
      ...
      const response = await request(app)
        .get('/hosts/mac-vendor/AA:BB:CC:DD:EE:FF')
        .expect(200);
    ...
    it('should handle rate limiting', async () => {
      ...
      const response = await request(app)
        .get('/hosts/mac-vendor/BB:BB:CC:DD:EE:FF')
        .expect(429);
    ...
    it('should cache vendor lookups', async () => {
      ...
      // First request
      await request(app)
        .get('/hosts/mac-vendor/CC:CC:CC:DD:EE:FF')
        .expect(200);
  ...
  describe('Error handling', () => {
    ...
    it('should handle invalid routes', async () => {
      await request(app)
        .get('/hosts/invalid/route/structure')
        .expect(404);
    });
  });
});
