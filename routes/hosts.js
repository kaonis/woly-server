...
var db = new sqlite3.Database('./db/woly.db', (err) => {
    if (err) {
...
router.get('/:name', function (req, res) {
    ...
    //SQL statement
    var sql = `SELECT hosts.name name, hosts.mac mac, hosts.ip ip, hosts.status status
           FROM hosts
           WHERE name  = ?`;
...
router.get('/wakeup/:name', function (req, res) {
    var name = req.params.name;
    ...
    db.get(sql, [name], (err, row) => {
        ...
        wol.wake(row.mac, function (error) {
            if (error) {
        ...
        //var magic_packet = wol.createMagicPacket(row.mac);
        return row;
    });
});
...
db.run(
    ...
    function (err) {
        for (var i = 0; i < hostTable.length; i++) {
            db.run(`INSERT INTO hosts("name", "mac", "ip", "status") VALUES(?,?,?,?)`,
                hostTable[i][0], hostTable[i][1], hostTable[i][2], hostTable[i][3],
                function (err) {
                    if (err) {
