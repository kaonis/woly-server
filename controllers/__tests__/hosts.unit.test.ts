...
describe('hosts controller', () => {
  ...
  describe('getAllHosts', () => {
    it('should return all hosts successfully', async () => {
      const mockHosts = [
        { name: 'Host1', mac: 'AA:BB:CC:DD:EE:FF', ip: '192.168.1.1', status: 'awake', lastSeen: null, discovered: 1 }
      ];
      mockDb.getAllHosts.mockResolvedValue(mockHosts as any);

      await hostsController.getAllHosts(mockReq as Request, mockRes as Response, mockNext);

      expect(mockDb.getAllHosts).toHaveBeenCalled();
      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(mockRes.json).toHaveBeenCalledWith({ hosts: mockHosts });
    });
    ...
    it('should return empty array when no hosts exist', async () => {
      ...
      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(mockRes.json).toHaveBeenCalledWith({ hosts: [] });
    });
    ...
    it('should handle database errors', async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({ error: 'Failed to fetch hosts' });
    });
  ...
  describe('getHost', () => {
    it('should return specific host by name', async () => {
      const mockHost = { name: 'Host1', mac: 'AA:BB:CC:DD:EE:FF', ip: '192.168.1.1', status: 'awake', lastSeen: null, discovered: 1 };
      mockReq.params = { name: 'Host1' };
      mockDb.getHost.mockResolvedValue(mockHost as any);

      await hostsController.getHost(mockReq as Request, mockRes as Response, mockNext);
    ...
    it('should handle database lookup errors', async () => {
      ...
      expect(mockRes.status).toHaveBeenCalledWith(500);
      expect(mockRes.json).toHaveBeenCalledWith({ error: 'Failed to fetch host' });
    });
  });

  describe('wakeUpHost', () => {
    it('should send WoL packet successfully', async () => {
      const mockHost = { name: 'Host1', mac: 'AA:BB:CC:DD:EE:FF', ip: '192.168.1.1', status: 'asleep', lastSeen: null, discovered: 1 };
      mockReq.params = { name: 'Host1' };
      mockDb.getHost.mockResolvedValue(mockHost as any);
      
      // Mock WoL success
      (wol.wake as jest.Mock).mockImplementation((mac: string, callback: (err: Error | null) => void) => {
        callback(null);
      });

      await hostsController.wakeUpHost(mockReq as Request, mockRes as Response, mockNext);
  ...
  describe('scanNetwork', () => {
    it('should trigger network scan and return results', async () => {
      ...
      await hostsController.scanNetwork(mockReq as Request, mockRes as Response, mockNext);
    ...
    it('should handle network scan errors', async () => {
      ...
      expect(mockRes.status).toHaveBeenCalledWith(500);
      expect(mockRes.json).toHaveBeenCalledWith({ error: 'Failed to scan network' });
    });
  });
  ...
  describe('addHost', () => {
    it('should add new host with valid data', async () => {
      ...
      await hostsController.addHost(mockReq as Request, mockRes as Response, mockNext);
    ...
    it('should reject request with missing fields', async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({ error: 'Missing required fields: name, mac, ip' });
    });
    ...
    it('should handle duplicate host errors', async () => {
      ...
      expect(mockRes.status).toHaveBeenCalledWith(500);
      expect(mockRes.json).toHaveBeenCalledWith({ error: 'Failed to add host' });
    });
  });
  ...
  describe('getMacVendor', () => {
    ...
    it('should fetch vendor from API when not cached', async () => {
      ...
      await hostsController.getMacVendor(mockReq as Request, mockRes as Response, mockNext);
